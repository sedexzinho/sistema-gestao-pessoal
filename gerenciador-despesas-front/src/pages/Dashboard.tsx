...
export function Dashboard() {
  ...
  const totalReceitas = receitas
    .filter((r) => {
      if (r.statusReceita !== "RECEBIDO") return false;
      const data = new Date(r.dataRecebimentoReceita);
  ...
  const totalParceladas = despesas
    .filter((d) => {
      if (d.tipo !== "PARCELADO" || d.status === "CONCLUIDO") return false;
      if (!d.dataRegistro || !d.diaPagamento) return false;

      const dataRegistro = new Date(d.dataRegistro);
      const primeiroVencimento = new Date(dataRegistro);
      primeiroVencimento.setMonth(primeiroVencimento.getMonth() + 1);

      const mesesDesdeInicio =
        (anoAtual - primeiroVencimento.getFullYear()) * 12 +
        (mesAtual - primeiroVencimento.getMonth());

      if (mesesDesdeInicio < 0) return false;
      if (mesesDesdeInicio >= (d.totalParcelas ?? 0)) return false;

      const vencimentoEsseMes = new Date(anoAtual, mesAtual, d.diaPagamento);
      return hoje >= vencimentoEsseMes;
    })
    .reduce((acc, d) => acc + (d.valorParcela ?? 0), 0);

  const parcelasPendentes = despesas.filter(
    (d) => d.tipo === "PARCELADO" && d.status !== "CONCLUIDO",
  ).length;

  // ✅ Gráfico com dados reais dos últimos 6 meses
    ...
    return Array.from({ length: 6 }, (_, i) => {
      ...
      const receitasMes = receitas
        .filter((r) => {
          if (r.statusReceita !== "RECEBIDO") return false;
          const data = new Date(r.dataRecebimentoReceita);
          return data.getMonth() === m && data.getFullYear() === a;
        })
        .reduce((acc, r) => acc + r.valorReceita, 0);

      const despesasMes = despesas
        .filter((dep) => {
          const data = new Date(dep.dataRegistro);
          return data.getMonth() === m && data.getFullYear() === a;
        })
          ...
          <CardResumo
            ...
            valor={formatarMoeda(
              despesas
                ...
                .reduce((acc, d) => {
                  const restantes =
                    (d.totalParcelas ?? 0) - (d.parcelaAtual ?? 0);
                ...
                <div key={d.id} style={styles.parcelaItem}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={styles.parcelaInfo}>
                      {d.parcelaAtual}/{d.totalParcelas} • vence dia{" "}
                      {d.diaPagamento}
                    </p>
                  </div>
                  <p style={styles.parcelaValor}>
                    {formatarMoeda(d.valorParcela ?? 0)}
                  </p>
                </div>
              ))
            )}
